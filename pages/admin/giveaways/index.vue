<template>
    <AdminLayout>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6 mt-6">
            ...
            <div v-else-if="filteredGiveaways?.length === 0 && isLoading" class="col-span-full flex items-center justify-center mt-12">
                <Loader size="xl" />
            </div>

            <div v-else-if="filteredGiveaways?.length === 0 && !isLoading" class="col-span-full font-bold mt-12">
                <img src="/icons/empty-giveaways.svg" alt="Empty Giveaways" class="w-96 mx-auto" />
                ...
                <div class="w-full mx-auto text-center items-center justify-center">
                    <Button @click="handleOpenCreateGiveaway" class="mt-6">
                        <Gift class="w-5 h-5 text-white" />
                        Створити розіграш
                    </Button>
                </div>
